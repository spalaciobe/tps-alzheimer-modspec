...
\vfill
\begin{block}{Y a primera vista\ldots}
\small STFT parecía \textbf{superar} a CWT
(DeLong $p = 0.014$). \\
\textit{Titular tentador: ``STFT gana''.}
\end{block}
\end{column}
\end{columns}
...
\vfill
\centering\small
\textbf{Replicación funcional: exitosa.} Pero antes de anunciar que
STFT gana, había que hacer bien la estadística.
...
\small El $p=0.014$ se calculaba sobre la \emph{mediana} entre seeds
(un ensemble que infla la señal). Haciéndolo bien --- DeLong
\textbf{por seed} + combinación:
...
\begin{tabular}{l c c}
\toprule
Comparación (vainilla) & $p$ correcto & BH-FDR $q$ \\
\midrule
