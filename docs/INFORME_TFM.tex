...
\begin{figure}[H]
    ...
    (STFT $\mid$ CWT nativa $\mid$ CWT-fair). Al igualar el eje de
    modulación, la \textbf{CWT-fair pierde el ``cono''} de la wavelet
    --- visible en la CWT nativa como energía que se extiende hasta
    modulaciones altas --- y su estructura de modulación se acerca a la
    de la STFT: evidencia visual del control DSP. \emph{Nota honesta}:
    esto iguala el eje de modulación (horizontal), \emph{no} el de
    portadora (vertical); por eso convergen las métricas (§\ref{sec:delong_per_seed})
    pero \emph{no} necesariamente los mapas de saliency (§\ref{sec:sal_ranking}).}
    \label{fig:fair_cone}
\end{figure}
...
\begin{enumerate}[leftmargin=*]
    ...
    \begin{itemize}[leftmargin=*]
        ...
        cone-of-influence).
        \item \textbf{Confirmar la complementariedad del late-fusion
        STFT+CWT-fair}: el indicio preliminar (AUC $0.867 \pm 0.014$ vs
        STFT 0.856, §4.9) es prometedor pero pequeño, inconsistente entre
        seeds y no se sostiene en el ensemble mediana; validar con
        $\geq10$ seeds y un test formal. Sería el camino más directo para
        convertir el resultado nulo (STFT$\approx$CWT) en un hallazgo
        positivo.
...
(\texttt{SubjectBank}, fp16-bank, GC entre folds), revisión
metodológica y redacción de este informe fueron
