...
\begin{enumerate}[leftmargin=*]
    ...
    vainilla, AUC media de STFT supera la de CWT, pero la inferencia
    formal (DeLong AUC por seed + combinación Stouffer/Fisher)
    da $p \approx 0.061$ (NS), y tras BH-FDR sobre las 3~comparaciones
    principales \emph{ninguna} sobrevive ($q \geq 0.10$).
...
\textbf{DeLong por seed} (combinado Stouffer): $\Delta\text{AUC}_{\text{STFT}-\text{CWT}} \approx +0.10$, $p = 0.266$ (NS).
...
\textbf{DeLong por seed} (combinado Stouffer): $\Delta\text{AUC}_{\text{CWT}-\text{STFT}} \approx +0.09$, $p = 0.068$ (NS).
...
\subsubsection{Inferencia estadística: DeLong por seed + combinación}
\label{sec:delong_per_seed}

La comparación de AUC entre STFT y CWT se hace con DeLong pareado
\emph{dentro de cada seed por separado} (n=65~sujetos en cada test
LOSO), combinando luego los tres $p$-values entre seeds con Stouffer y
Fisher. Es el procedimiento correcto: preserva la variabilidad real
entre inicializaciones en lugar de colapsarla en un ensemble.
...
Ningún seed individual alcanza significancia al $\alpha = 0.05$, y la
combinación tampoco ($p \approx 0.061$). El AUC medio mayor de STFT y su
menor varianza son consistentes con una posible ventaja, pero la
inferencia formal \textbf{no alcanza significancia}.
...
Las tres comparaciones principales (CNN, SVM Grad-CAM, SVM vainilla) se
corrigen por multiplicidad con Benjamini-Hochberg ($\alpha=0.05$) sobre
los $p$-values combinados (Stouffer):
...
\textbf{Ninguna comparación sobrevive BH-FDR} ($q \geq 0.10$). La lectura
definitiva es \textbf{ausencia de evidencia concluyente para superioridad
de cualquier método T-F en este pipeline}.

\subsubsection{Wilcoxon por seed individual (consistencia)}

Wilcoxon pareado de scores por sujeto en cada seed individual
(SVM vainilla, STFT vs CWT) corrobora la falta de un efecto consistente:
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{DeLong AUC por seed + combinación} (SVM vainilla):
    $p$ por seed $= 0.660, 0.051, 0.072$; Stouffer/Fisher combinado
    $= 0.061$ (NS); tras BH-FDR ninguna comparación sobrevive
